...
class IssueTrackerDisableUpdater(IssueTrackerCommentUpdater):
  """This class should sync comments added to issuetracked objects."""
  ...
  def sync_issuetracker(self, request_data):
    ...
    Args:
        request_data: {
            'objects_to_comment': {'object_type': [obj_ids]},
            'mail_data': {user_email: email}
        }
        Disabled issuetracker issues list contains ids of objects
        to by synchronized.
    Returns:
        flask.wrappers.Response - response with result of generation.
    """
  ...
  def _get_issue_json(self, object_):
    ...
    issue_json = None
    integration_handler = self.INTEGRATION_HANDLERS.get(object_.type)
    if hasattr(integration_handler, "prepare_disable_comment_json"):
  ...
  @staticmethod
  def get_issuetracked_objects(objects_to_comment):
    """Fetch issuetracked objects from db."""
    objects = []
    for obj_type, obj_ids in objects_to_comment.items():
      issuetracked_model = inflector.get_model(obj_type)
